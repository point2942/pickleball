...
router.get('/schedule', async (req, res) => {
  ...
  const dow = d.getDay();
  const { data: blocked } = await supabase.from('blocked_slots').select('*').eq('is_active', true);
  const blockedForDate = (blocked || []).filter(b => {
    if (b.block_date) return b.block_date === date;
    if (b.day_of_week === dow) {
      const afterFrom = !b.valid_from || date >= b.valid_from;
      const beforeUntil = !b.valid_until || date <= b.valid_until;
      return afterFrom && beforeUntil;
    }
    return false;
  });

  res.json({ bookings: data, blockedSlots: blockedForDate });
});
...
router.post('/', authMiddleware, async (req, res) => {
  ...
  // 檢查每個時段是否有封場設定
  const { data: allBlocked } = await supabase.from('blocked_slots').select('*').eq('is_active', true);
  for (const s of slots) {
    const d = new Date(s.date + 'T00:00:00');
    const dow = d.getDay();
    const isBlocked = (allBlocked || []).some(b => {
      const _dow = new Date(s.date + 'T00:00:00').getDay();
      const dateMatch = b.block_date === s.date || (b.day_of_week === _dow && !b.block_date && (!b.valid_from || s.date >= b.valid_from) && (!b.valid_until || s.date <= b.valid_until));
      const hourMatch = s.hour >= b.hour_start && s.hour <= b.hour_end;
      const courtMatch = b.court_ids.includes(s.court_id);
      return dateMatch && hourMatch && courtMatch;
    });
    if (isBlocked) {
      return res.status(400).json({ error: `場地 ${s.court_id} 在 ${s.date} ${String(s.hour).padStart(2,'0')}:00 已封場，無法預約` });
    }
  }
