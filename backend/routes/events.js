...
// ── 查詢活動的報名名單（公開，僅回傳姓名，不含電話等隱私資訊） ──
router.get('/:id/roster', async (req, res) => {
  const { data: regs } = await supabase
    .from('event_registrations').select('guest_name, status, queue_position')
    .eq('event_id', req.params.id)
    .in('status', ['confirmed', 'waitlist'])
    .order('status').order('queue_position').order('created_at');

  const confirmed = (regs || []).filter(r => r.status === 'confirmed').map(r => r.guest_name);
  const waitlist = (regs || []).filter(r => r.status === 'waitlist').map(r => r.guest_name);
