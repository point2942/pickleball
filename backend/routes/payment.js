...
router.post('/ecpay/event-callback', async (req, res) => {
  const data = req.body;
  const received = data.CheckMacValue;
  delete data.CheckMacValue;
  const expected = genECPayMac(data);
  ...
  const regId = data.CustomField1;
  const { data: reg } = await supabase.from('event_registrations').select('*').eq('id', regId).single();
  if (!reg || reg.status === 'confirmed') return res.send('1|OK'); // 避免重複處理

  const { data: event } = await supabase.from('events').select('*').eq('id', reg.event_id).single();

  await supabase.from('event_registrations').update({
    status: 'confirmed', paid_at: new Date()
  }).eq('id', reg.id);

  await sendLineNotify(`✅ 活動報名付款成功（綠界）\n活動：${event?.title}\n姓名：${reg.guest_name}（${reg.guest_phone}）`);
  if (event) {
    const { count } = await supabase.from('event_registrations').select('*', { count: 'exact', head: true })
      .eq('event_id', event.id).eq('status', 'confirmed');
    if (count >= event.capacity) await sendLineNotify(`📢 活動名額已滿\n活動：${event.title}`);
  }
  res.send('1|OK');
});

router.get('/linepay/event-confirm', async (req, res) => {
  const { transactionId, orderId } = req.query;

  const { data: reg } = await supabase.from('event_registrations').select('*').eq('payment_ref', orderId).single();
  if (!reg) return res.redirect('/payment-result?status=error');
  if (reg.status === 'confirmed') return res.redirect('/payment-result?status=success');

  const { data: event } = await supabase.from('events').select('*').eq('id', reg.event_id).single();
  const amount = reg.fee - (reg.points_used || 0);
  ...
  try {
    const resp = await axios.post(`${process.env.LINEPAY_API_URL}${path}`, body, {
    ...
    if (resp.data.returnCode === '0000') {
      await supabase.from('event_registrations').update({ status: 'confirmed', paid_at: new Date() }).eq('id', reg.id);
      await sendLineNotify(`✅ 活動報名付款成功（LINE Pay）\n活動：${event?.title}\n姓名：${reg.guest_name}（${reg.guest_phone}）`);
      if (event) {
        const { count } = await supabase.from('event_registrations').select('*', { count: 'exact', head: true })
          .eq('event_id', event.id).eq('status', 'confirmed');
        if (count >= event.capacity) await sendLineNotify(`📢 活動名額已滿\n活動：${event.title}`);
