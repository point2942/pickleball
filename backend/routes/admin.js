...
router.delete('/bookings/:id', adminMiddleware, async (req, res) => {
  ...
  // 若該筆訂單先前有折抵點數，取消時退回點數（不論原狀態是 pending 或 paid，只要曾折抵就該退回）
  if (booking.points_used > 0) {
    await creditPoints({
      memberId: booking.member_id, points: booking.points_used, type: 'refund',
      note: `管理員取消預約退回點數（場地${booking.court_id} ${booking.date} ${String(booking.hour).padStart(2,'0')}:00）`,
...
router.get('/members', adminMiddleware, async (req, res) => {
  const { data } = await supabase.from('members').select('*').order('phone', { ascending: true });
...
router.get('/topup-tiers', adminMiddleware, async (req, res) => {
  const { data } = await supabase.from('topup_tiers').select('*').order('sort_order');
  res.json({ tiers: data || [] });
});

router.post('/topup-tiers', adminMiddleware, async (req, res) => {
  const { min_amount, max_amount, multiplier, label, sort_order } = req.body;
  if (!min_amount || !multiplier) return res.status(400).json({ error: '請填寫最低金額與倍率' });
  const { data, error } = await supabase.from('topup_tiers')
    .insert({ min_amount, max_amount: max_amount || null, multiplier, label, sort_order: sort_order || 0 })
    .select().single();
  if (error) return res.status(400).json({ error: error.message });
  res.json({ tier: data });
});

router.put('/topup-tiers/:id', adminMiddleware, async (req, res) => {
  const { min_amount, max_amount, multiplier, label, sort_order, is_active } = req.body;
  await supabase.from('topup_tiers').update({
    min_amount, max_amount: max_amount || null, multiplier, label, sort_order, is_active
  }).eq('id', req.params.id);
  res.json({ ok: true });
});

router.delete('/topup-tiers/:id', adminMiddleware, async (req, res) => {
  await supabase.from('topup_tiers').delete().eq('id', req.params.id);
  res.json({ ok: true });
});
...
// 查詢所有封場設定
router.get('/blocked-slots', adminMiddleware, async (req, res) => {
  const { data } = await supabase.from('blocked_slots').select('*').order('created_at', { ascending: false });
  res.json({ slots: data || [] });
});

// 新增封場時段
router.post('/blocked-slots', adminMiddleware, async (req, res) => {
  const { label, court_ids, hour_start, hour_end, block_date, day_of_week, valid_from, valid_until } = req.body;
  if (!court_ids?.length) return res.status(400).json({ error: '請選擇要封場的場地' });
  if (hour_start == null || hour_end == null) return res.status(400).json({ error: '請填寫封場時段' });
  if (!block_date && day_of_week == null) return res.status(400).json({ error: '請選擇一次性日期或每週幾' });
  ...
  const { data, error } = await supabase.from('blocked_slots').insert({
    ...
    day_of_week: block_date ? null : day_of_week,
    valid_from: (!block_date && valid_from) ? valid_from : null,
    valid_until: (!block_date && valid_until) ? valid_until : null
  }).select().single();
  if (error) return res.status(500).json({ error: error.message });
  res.json({ slot: data });
});
...
// 啟用/停用封場設定
router.patch('/blocked-slots/:id', adminMiddleware, async (req, res) => {
  const { is_active } = req.body;
  await supabase.from('blocked_slots').update({ is_active }).eq('id', req.params.id);
  res.json({ ok: true });
});

// 刪除封場設定
router.delete('/blocked-slots/:id', adminMiddleware, async (req, res) => {
  await supabase.from('blocked_slots').delete().eq('id', req.params.id);
  res.json({ ok: true });
});
